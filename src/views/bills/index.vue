<template>
  <app-view size="medium" title="Bills">
    <div slot="option">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="item in filteredItems" :key="item.id">
                <td class="px-6 py-2 whitespace-no-wrap">
                  <div class="text-sm leading-5 text-gray-900">{{item.customer.name}}</div>
                </td>
                <td class="px-6 py-2 whitespace-no-wrap">
                ...
                <td class="px-6 py-2 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                  <button @click="$router.push({name: 'BillUpdate', params: { id: item.id }})" class="mx-1 font-bold h-8 w-8 justify-center rounded-full inline-flex items-center bg-green-300 hover:bg-green-400 text-green-800">
